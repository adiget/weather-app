package com.ags.annada.weatherforecastapp.model;
...
import com.ags.annada.weatherforecastapp.retrofit.NetworkError;
import com.ags.annada.weatherforecastapp.retrofit.Service;
...
public class MainModel implements MVP_Main.ProvidedModelOps {
    ...
    private void makeForecastRequiredModel(WeatherForecast forecast) {
        ...
        for (int i = 0; i < weatherInfo.size(); i++) {
            ...
            String icon = weatherInfo.get(i).getWeather().get(0).getIcon();

            if (convertTimeToDay(time).equals("Mon") && everyday[0] < 1) {
                mForecastData.add(new ForecastRequiredData(shortDay, icon, temp, tempMin));
                everyday[0] = 1;
            }
